...
parser = argparse.ArgumentParser(description='pytorch training vae')
...
def main():
    ...
    if args.dataset == "lung":
        ...
        model = vae.VAE3d(latent_space_dim=args.latent_dim)
    ...
    writer_log_dir = "/data/fhz/unsupervised_recommendation/vae_runs/vae_train_time:{}_dataset:{}".format(
    ...
    if args.resume:
        if os.path.isfile(args.resume):
            ...
            model.z_mean_map.load_state_dict(checkpoint['z_mean_map_state_dict'])
            model.z_log_sigma_map.load_state_dict(checkpoint['z_sigma_map_state_dict'])
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if (epoch + 1) % 5 == 0:
            ...
            save_checkpoint({
                ...
                'z_mean_map_state_dict': model.z_mean_map.state_dict(),
                'z_sigma_map_state_dict': model.z_log_sigma_map.state_dict(),
            ...
            if (epoch + 1) > 300:
                test_datalist = sample(train_datalist, 10)
                comment = ""
...
def train(train_dloader, model, criterion, optimizer, epoch, writer, dataset):
    ...
    for i, (image, *_) in enumerate(train_dloader):
        ...
        reconstruct_loss, kl_loss = criterion(image, image_reconstructed, model.z_mean, model.z_log_sigma,
                                              model.z_sigma)
...
def save_checkpoint(state, is_total_loss_best, is_reconstruct_loss_best, is_kl_loss_best,
    ...
    :return:
    """
    filefolder = '/data/fhz/unsupervised_recommendation/vae_parameter/unsupervised_recommendation_train_vae_time_{}_train_dset_{}'.format(
...
def test(test_datalist, model, train_time, train_epoch, dataset, comment, criterion):
    filefolder = '/data/fhz/unsupervised_recommendation/unsupervised_recommendation_train_vae_time_{}_train_dset_{}/test_{}_epoch/{}'.format(
...
def inference(model, train_datalist, test_datalist, folder_path=None):
    if folder_path is None:
        folder_path = '/data/fhz/unsupervised_recommendation/vae_inference'
        folder_path = os.path.join(folder_path, "train_time_{}_dset_{}".format(args.train_time,args.dataset))
    if not os.path.exists(folder_path):
        os.makedirs(folder_path)
    if args.dataset == "lung":
        resume_path = "/data/fhz/unsupervised_recommendation/vae_parameter/unsupervised_recommendation_train_vae_time_{}_train_dset_lung/model_total_loss_best.pth.tar".format(
            args.train_time)
    elif args.dataset == "gland":
        resume_path = "/data/fhz/unsupervised_recommendation/vae_parameter/unsupervised_recommendation_train_vae_time_{}_train_dset_gland/model_total_loss_best.pth.tar".format(
            args.train_time)
    else:
    ...
    for i, (image, index, img_name, *_) in enumerate(train_dloader):
        ...
        mu = model.z_mean_map(features)
        log_sigma = model.z_log_sigma_map(features)
        mu = mu.cpu().detach().numpy()
        log_sigma = log_sigma.cpu().detach().numpy()
        train_inference[img_name] = {"mu": mu, "log_sigma": log_sigma}
    ...
    for i, (image, index, img_name, *_) in enumerate(test_dloader):
        ...
        mu = mu.cpu().detach().numpy()
        log_sigma = log_sigma.cpu().detach().numpy()
        test_inference[img_name] = {"mu": mu, "log_sigma": log_sigma}
        print("{} inferenced".format(img_name))
